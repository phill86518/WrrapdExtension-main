...
function wrrapd_inject_header_cta_rules() {
	...
	echo '<style id="wrrapd-mobile-auth-links-css">@media (max-width:900px){#wrrapd-mobile-auth-links{display:flex;gap:8px;align-items:center;justify-content:flex-end;padding:8px 12px;background:#0b0b78;border-top:1px solid rgba(255,255,255,.15);border-bottom:1px solid rgba(255,255,255,.15);}#wrrapd-mobile-auth-links a{display:inline-flex;align-items:center;justify-content:center;min-height:30px;padding:0 12px;border-radius:999px;background:#f6c343;color:#0b0b78!important;font:700 12px/1.1 Arial,sans-serif;text-decoration:none!important;box-shadow:0 1px 2px rgba(0,0,0,.25);}}</style>';
	echo '<script>(function(){function wrNorm(t){return (t||"").replace(/\\s+/g," ").trim().toLowerCase();}function wrFindLink(root,kind){var out=null;root.querySelectorAll("a[href]").forEach(function(a){if(out)return;var txt=wrNorm(a.textContent);if(kind==="account"&&(txt==="account"||txt==="my account"||txt==="my-account"))out=a;if(kind==="logout"&&(txt==="logout"||txt==="log out"||txt.indexOf("logout")!==-1))out=a;});return out;}function wrInject(){if(window.innerWidth>900)return;if(document.getElementById("wrrapd-mobile-auth-links"))return;var hdr=document.querySelector("header, .site-header, .elementor-location-header");if(!hdr)return;var account=wrFindLink(document,"account");var logout=wrFindLink(document,"logout");if(!account||!logout)return;var wrap=document.createElement("div");wrap.id="wrrapd-mobile-auth-links";var a1=document.createElement("a");a1.href=account.href;a1.textContent="Account";var a2=document.createElement("a");a2.href=logout.href;a2.textContent="Logout";wrap.appendChild(a1);wrap.appendChild(a2);hdr.insertAdjacentElement("afterend",wrap);}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",wrInject,{once:true});}else{wrInject();}window.addEventListener("resize",wrInject);setTimeout(wrInject,600);})();</script>';
