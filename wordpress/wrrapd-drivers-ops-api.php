...
function wrrapd_drivers_run_admin_action( $app_id, $action, $opts = array() ) {
	...
	if ( $action === 'save_gig_evidence' ) {
		...
		wrrapd_drivers_set_meta( $app_id, 'gig_evidence_status', $status );
		wrrapd_drivers_set_meta( $app_id, 'gig_evidence_notes', sanitize_textarea_field( (string) ( $opts['gig_evidence_notes'] ?? '' ) ) );
		wrrapd_drivers_set_meta( $app_id, 'gig_evidence_at', gmdate( 'c' ) );
		return array( 'ok' => true, 'status' => wrrapd_drivers_get_meta( $app_id, 'status' ) );
	}
...
function wrrapd_drivers_ops_serialize_application( $id ) {
	...
	return array(
		...
		'deliveryGigPlatforms'    => wrrapd_drivers_get_meta( $id, 'delivery_gig_platforms' ),
		'fitScore'                => (int) wrrapd_drivers_get_meta( $id, 'fit_score', '0' ),
		'fitBreakdown'            => wrrapd_drivers_get_meta( $id, 'fit_breakdown' ),
		'gigEvidenceStatus'       => wrrapd_drivers_get_meta( $id, 'gig_evidence_status' ),
		'gigEvidenceNotes'        => wrrapd_drivers_get_meta( $id, 'gig_evidence_notes' ),
		'gigEvidenceAt'           => wrrapd_drivers_get_meta( $id, 'gig_evidence_at' ),
		...
		'esignSuite'              => wrrapd_drivers_get_meta( $id, 'esign_suite' ),
		'esignVersion'            => wrrapd_drivers_get_meta( $id, 'esign_version' ),
		'esignAcceptedAt'         => wrrapd_drivers_get_meta( $id, 'esign_accepted_at' ),
		'esignTypedName'          => wrrapd_drivers_get_meta( $id, 'esign_typed_name' ),
		'esignIp'                 => wrrapd_drivers_get_meta( $id, 'esign_ip' ),
		'esignUa'                 => wrrapd_drivers_get_meta( $id, 'esign_ua' ),
		'esignDocs'               => wrrapd_drivers_get_meta( $id, 'esign_docs' ),
		'esignDocAcks'            => wrrapd_drivers_get_meta( $id, 'esign_doc_acks' ),
		'esignMethod'             => wrrapd_drivers_get_meta( $id, 'esign_method' ),
		'icSignedAt'              => wrrapd_drivers_get_meta( $id, 'ic_signed_at' ),
		'hasIdFile'               => (bool) wrrapd_drivers_get_meta( $id, 'id_file' ),
		'hasDrivingAbstractFile'  => (bool) wrrapd_drivers_get_meta( $id, 'driving_abstract_file' ),
